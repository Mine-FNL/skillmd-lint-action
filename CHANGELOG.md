...
- CI: `test.yml` used expressions in `uses:`, which GitHub rejects
  before any job starts; it now exercises the Docker action via
  `uses: ./` with output assertions. `ci.yml` now runs the official
  hadolint action and a plain `docker run` for actionlint.
